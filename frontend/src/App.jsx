...
const MOCK_PROJECTS = [
  {
    ...
    notes: [
      ...
      "Ma'am suggested using a prebuilt base to skip the kernel phase entirely, not sure about that.",
    ],
    tasks: [
  ...
  {
    id: 'p3', title: 'JEE Preparation',
    summary: 'Develop a one-month study plan for JEE Physics preparation.',
    deadline: '1 month', status: 'active', notes: [],
    tasks: [
...
export default function App() {
  ...
  useEffect(() => {
    createSession()
      .then((s) => { setSessionId(s.id); console.log('ADK session:', s.id); })
  ...
  const sendToAgent = async (text) => {
    ...
    try {
      ...
      if (reply) setChatMessages((p) => [...p, { role: 'assistant', text: reply }]);
    } catch (err) {
      console.error('Agent error:', err);
      setChatMessages((p) => [...p, { role: 'assistant', text: '⚠️ Something went wrong. Please try again.' }]);
    } finally {
  ...
  const renderContent = () => {
    ...
    if (activeTab === 'dashboard') return <Dashboard onSelectProject={setSelectedProject} />;
    return <CalendarView />;
  };
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 transition-all duration-300">
        <nav className="flex items-center gap-1 px-6 py-4 border-b border-borderLight bg-white">
          <NavTab icon={<LayoutDashboard size={18} />} label="Dashboard"
            active={activeTab === 'dashboard' && !selectedProject}
            onClick={() => { setActiveTab('dashboard'); setSelectedProject(null); }} />
          <NavTab icon={<CalendarDays size={18} />} label="Calendar"
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <div className="w-8 h-8 rounded-full bg-accent text-white flex items-center justify-center text-sm font-medium">A</div>
          </div>
        </nav>
        ...
        <div className="px-6 pb-6 pt-2">
          <div className="max-w-2xl mx-auto">
            <div className="flex items-center bg-white border border-borderLight rounded-full shadow-card px-5 py-1 focus-within:border-borderFocus focus-within:shadow-cardHover transition-all">
              ...
              <input type="text" value={bottomInput}
              ...
              <button onClick={handleBottomSend}
      ...
      <aside className={cn(
          ...
          {chatMessages.map((msg, i) =>
            msg.role === 'user' ? (
              <div key={i} className="self-end max-w-[80%]">
                <div className="bg-accent text-white px-4 py-2.5 rounded-2xl rounded-br-md text-sm leading-relaxed">{msg.text}</div>
              </div>
            ) : (
              <div key={i} className="self-start max-w-[90%]">
                <div className="bg-surface px-4 py-3 rounded-2xl rounded-bl-md text-sm leading-relaxed text-textPrimary whitespace-pre-line">{msg.text}</div>
              </div>
            )
          )}
          {isAgentThinking && (
        ...
        <div className="p-4 border-t border-borderLight flex-shrink-0">
          <div className="flex items-center bg-surface rounded-full px-4 py-1 border border-borderLight focus-within:border-borderFocus transition-colors">
            ...
            <button onClick={handleChatSend} disabled={isAgentThinking}
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {MOCK_PROJECTS.map((p) => <ProjectCard key={p.id} project={p} onClick={() => onSelectProject(p)} />)}
      </div>
    </div>
  );
}
  ...
  return (
    <div onClick={onClick}
      ...
      <div className="flex items-center gap-2 mb-3">
        <span className="inline-flex items-center gap-1 bg-tagBg text-tagText text-xs font-medium px-2.5 py-1 rounded-full">📋 Project</span>
        {project.status === 'completed' && (
          <span className="inline-flex items-center gap-1 bg-successBg text-successText text-xs font-medium px-2.5 py-1 rounded-full">✓ Completed</span>
        )}
      </div>
      ...
      <div className="flex items-center justify-between mt-auto pt-3 border-t border-borderLight">
        <div className="flex items-center gap-4 text-xs text-textTertiary">
          {project.deadline && <span className="flex items-center gap-1"><Clock size={13} />{project.deadline}</span>}
      ...
      <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="bg-surfaceCard rounded-card border border-borderLight p-5 space-y-4">
              {project.notes.map((n, i) => (
                <div key={i} className="flex gap-3 text-sm text-textSecondary leading-relaxed">
                  <span className="text-textTertiary font-medium flex-shrink-0">{i + 1}.</span><p>{n}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className={cn("w-full", project.notes.length > 0 ? "lg:w-1/2" : "lg:w-2/3")}>
          ...
          <div className="space-y-3">
            {project.tasks.map((t) => (
              <div key={t.id} className={cn("flex items-center gap-3 bg-surfaceCard rounded-card border border-borderLight px-5 py-3.5 transition-shadow hover:shadow-card", t.done && "bg-surface")}>
                {t.done ? <CheckCircle2 size={20} className="text-accent flex-shrink-0" /> : <Circle size={20} className="text-borderLight flex-shrink-0" />}
                <span className={cn("text-sm flex-1", t.done ? "text-textTertiary line-through" : "text-textPrimary")}>{t.title}</span>
              </div>
            ))}
          </div>
        </div>
...
function CalendarView() {
  ...
  const daysInMonth = 31, startDay = 3, today = 30;
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8">
      <div className="w-full lg:w-2/5">
        ...
        <div className="bg-surfaceCard rounded-card border border-borderLight p-5">
          <div className="grid grid-cols-7 mb-3">
            {dayNames.map((d) => <div key={d} className="text-center text-xs font-medium text-textTertiary py-2">{d}</div>)}
          </div>
          <div className="grid grid-cols-7 gap-y-1">
            {Array.from({ length: startDay }).map((_, i) => <div key={`e-${i}`} />)}
            {Array.from({ length: daysInMonth }).map((_, i) => {
              const day = i + 1, hasEv = MOCK_SCHEDULE[day] !== undefined;
              const isSel = day === selectedDay, isToday = day === today;
              return (
                <button key={day} onClick={() => setSelectedDay(day)}
                  className={cn("w-10 h-10 mx-auto rounded-full flex flex-col items-center justify-center text-sm font-medium relative transition-colors",
                    isSel ? "bg-accent text-white" : isToday ? "bg-accentLight text-accentText" : "text-textPrimary hover:bg-gray-100")}>
                  {day}
                  {hasEv && !isSel && <span className="absolute bottom-1 w-1 h-1 rounded-full bg-accent" />}
                </button>
              );
            })}
          </div>
      ...
      <div className="w-full lg:w-3/5">
        ...
        <p className="text-sm text-textSecondary mb-6">{selectedDay === today ? 'Today' : `August ${selectedDay}`}{schedule.length > 0 && ` · ${schedule.length} items`}</p>
        {schedule.length === 0 ? (
